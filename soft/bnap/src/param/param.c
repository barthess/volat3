...
extern EventSource event_mavlink_param_value;
...
static msg_t ParametersThread(void *arg){
  ...
  chEvtRegisterMask(&event_mavlink_param_set,           &el_param_set,          EVMSK_MAVLINK_PARAM_SET);
  chEvtRegisterMask(&event_mavlink_param_request_list,  &el_param_request_list, EVMSK_MAVLINK_PARAM_REQUEST_LIST);
  chEvtRegisterMask(&event_mavlink_param_request_read,  &el_param_request_read, EVMSK_MAVLINK_PARAM_REQUEST_READ);

  while (!chThdShouldTerminate()) {
    evt = chEvtWaitOne(EVMSK_MAVLINK_PARAM_SET | EVMSK_MAVLINK_PARAM_REQUEST_LIST | EVMSK_MAVLINK_PARAM_REQUEST_READ);

    switch (evt){
    /* set param */
    case EVMSK_MAVLINK_PARAM_SET:
      param_set_handler();
      break;
 ...
 *
 * @param[in] valuep  pointer to input value that must be checked and saved into
 * @param[in] paramp  poinster appropriate parameter in array
 *
 * @return            operation status.
 */
param_status_t set_global_param(void *valuep, const GlobalParam_t *paramp){

  if (paramp->func == NULL){
    switch(paramp->param_type){
    case MAVLINK_TYPE_FLOAT:
      return _float_param_checker(valuep, paramp);
      break;
    case MAVLINK_TYPE_UINT32_T:
      return _uint_param_checker(valuep, paramp);
      break;
    case MAVLINK_TYPE_INT32_T:
      return _int_param_checker(valuep, paramp);
      break;
    }
    ...
    return paramp->func(valuep, paramp);
}

/**
 * Check validity of parameter structure
 */
static void validate_param(void){
  int32_t i = 0;
  int32_t n = 0;
